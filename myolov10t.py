...
from ultralytics.nn.modules.block import PSAt, SpAMt, Bottleneckt, CIB,SCDown,SCUp,C2ft,C2fCIBt,SPPF
...
#         bs,ch,h,w=x.shape
#         x=torch.reshape(x,(bs,2,ch//2,h,w))
#         x=torch.permute(x,(0,2,1,3,4))
#         x = torch.reshape(x, (bs, ch, h, w))
#         return x
#
...
#         assert c1==2*c2
#         self.cv1=Conv(c2,c2,1,1,g=c2,act=False)
#         self.cv2 = Conv(c2, c2, 1, 1, g=c2, act=False)
#         self.up = nn.Upsample(scale_factor=2,mode='nearest')
#
#     def forward(self, x):
#         x0,x1=x[:,0::2,:,:],x[:,1::2,:,:]
#         return self.up(self.cv1(x0)+self.cv2(x1))
#
...
#         self.cv3 = nn.ModuleList(nn.Sequential(nn.Sequential(Conv(x, x, 5, g=x), Conv(x, x, 1)),
...
class v10DetectTiny(nn.Module):
    ...
    def forward_feat_(self, x, cv1, cv2, cv3):
        ...
        for i in range(self.nl):
            if self.export:
                e0=cv1[i](x[i]).sigmoid()
                e2=cv3[i](x[i]).softmax(1)
                if self.reg_max==1:
                    xywh=cv2[i](x[i])
                    xy=xywh[:,0:2,:,:].tanh()
                    wh=xywh[:,2:,:,:].sigmoid()
                    if e2.shape[1]==1:
                        y_ = torch.cat((e0, xy, wh), 1)
                    else:
                        y_=torch.cat((e0,xy ,wh, e2), 1)
                    ...
                    if e2.shape[1]==1:
                        y_ = torch.cat((e0, box), 1)
                    else:
                        y_=torch.cat((e0,box,e2), 1)
                bs, ch, h, w = y_.shape
